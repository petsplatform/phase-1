...
export const accountApi = {
  ...
  getRewards: async () => {
    const res = await api.get("/customer-panel/rewards");
    return res.data.data;
  },

  getPets: async () => {
    const res = await api.get("/customer-panel/pets");
    return res.data.data || [];
  },

  addPet: async (pet) => {
    const res = await api.post("/customer-panel/pets", pet);
    return res.data.data;
  },

  updatePet: async (id, pet) => {
    const res = await api.put(`/customer-panel/pets/${id}`, pet);
    return res.data.data;
  },

  removePet: async (id) => {
    const res = await api.delete(`/customer-panel/pets/${id}`);
    return res.data.data;
  },
