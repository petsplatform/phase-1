...
export function hasVariants(product) {
  ...
  if (Array.isArray(product.sizes) && product.sizes.length > 1)
    return true;
  if (Array.isArray(product.capacities) && product.capacities.length > 1)
    return true;
...
export function getProductUrl(product) {
  ...
  const targetId = product.slug || product.id || product.productId;
  if (isFamilyProduct(product)) {
    return `/product/${targetId}/variants`;
  }
