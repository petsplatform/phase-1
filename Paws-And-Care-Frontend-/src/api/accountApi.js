...
export const accountApi = {
  ...
  updateProfile: async (profile) => {
    const res = await api.patch('/customer-panel/profile', {
      ...profile,
  ...
  addAddress: async (address) => {
    const res = await api.post('/customer-panel/addresses', { address: normalizeAddressForApi(address) });
    return res.data.data || [];
  },
  updateAddress: async (index, address) => {
    const res = await api.put(`/customer-panel/addresses/${index}`, { address: normalizeAddressForApi(address) });
    return res.data.data || [];
  },
